...
mpz_t searchM(struct search_sol sol){
    ...
    mpz_t m, res1, res2, addM, zero, inter;
    mpz_init(zero);
    mpz_init(m);
    mpz_init(res1);
    mpz_init(res2);
...
alpha searchSolRec(struct search_sol sol){
    ...
    if(gmp_cmp_ui(sol.norme, 0) == 0){
	gmp_set_ui(newSol.alp.a, -1);
	gmp_set_ui(newSol.alp.b, -1);
	gmp_set(newSol.alp.n, sol.alp.n);
	gmp_set_ui(newSol.norme, 0);
	gmp_set(newSol.m, sol.m);
    ...
    gmp_abs(sol.norme, sol.norme);
    if(gmp_cmp_ui(sol.norme, 1)==0){
	alphaCarre(&sol.alp);
	return sol.alp;
    }
    else if(gmp_cmp_ui(sol.norme, 2)==0){
	alphaCarreSur2(&(sol.alp));
	return sol.alp;
    }
    else if(gmp_cmp_ui(sol.norme, 4)==0){
	mpz_t xa, xb, xn;
	...
	if(gmp_cmp_ui(xa, 0)==0 && gmp_cmp(xb, 0)){
	    if(gmp_cmp_ui(sol.norme, 0)>0){
	...
	else if(gmp_cmp(xn, 0)==0){
	    alphaCarreSur2(&(sol.alp));
	    gmp_div_ui(sol.alp.a, sol.alp.a, 2);
	    gmp_div_ui(sol.alp.b, sol.alp.b, 2);
	    return sol.alp;
	}
	else{
	    if(gmp_cmp_ui(sol.norme, 0)>0){
