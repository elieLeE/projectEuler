...
/* We want to gets all values below MAX_N (50 000 000) that can be write with
...
int main(void)
{
    const long max_n = 50000001;
    int *sols_count;
    long gap = 0;
    int res = 0;

    sols_count = p_calloc(sizeof(int) * max_n);

    while (true) {
        long x, sum, x_sum_max;
